...
public interface ProductoRepository extends BaseRepository<Producto, Long> {
    ...
    @Query(value = "select * from producto_manufacturado where estado = 0", nativeQuery = true)
    Page<Producto> findAllinAlta(Pageable page);

    @Query(value = "select * from producto_manufacturado where estado = 0", nativeQuery = true)
    List<Producto> findAllinAltaNoPage();

    @Query(value = "select * from producto_manufacturado p where estado = 0 and " +
    ...
    Page<Producto> filterProducts(@Param("id") Long id,
                                     @Param("nombre") String nombre,
